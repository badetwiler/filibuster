...
}}function o(aC,aB){var aA=aB;
if(aC.transport==="polling"){return aA
}if(c.trim(aB).length!==0&&aC.enableProtocol&&aC.firstMessage){var aD=aC.trackMessageLength?1:0;
var az=aB.split(aC.messageDelimiter);
if(az.length<=aD+1){return aA
}aC.firstMessage=false;
aC.uuid=c.trim(az[aD]);
aC.stime=c.trim(az[aD+1]);
b=false;
if(aC.transport!=="long-polling"){R(aC)
}c.atmosphere.uuid=aC.uuid;
aA="";
aD=aC.trackMessageLength?3:2;
if(az.length>aD+1){for(var ay=aD;
ay<az.length;
ay++){aA+=az[ay];
if(ay+1!==az.length){aA+=aC.messageDelimiter
}}}}else{if(aC.enableProtocol&&aC.firstMessage){}else{R(aC)
}}return aA
